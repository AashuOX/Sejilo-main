...
The legacy Tor integration is connected through `Sejilo/Nostr/` and the
`ChatViewModel+Tor.swift` extension in `Sejilo/ViewModels/Extensions/`. The
Windows/Android Flutter foundation does not currently include Tor support.
...
- **`TorManager`** — owns the Arti client and its data directory under Application Support, exposes the SOCKS port, and provides `awaitReady()`.
  - `torEnforced` is compile-time: true unless `SEJILOCHAT_DEV_ALLOW_CLEARNET` is defined. It is not set anywhere in `Configs/` or the project file, so release builds enforce.
...
`arti-client` is built with `default-features = false` and features `["tokio", "rustls"]` only — no `pt-client`, no `bridge-client` — and `arti-sejilochat/src/lib.rs` bootstraps from stock configuration with no bridge lines and no configurable directory authorities.
...
Define the Swift compiler flag `SEJILOCHAT_DEV_ALLOW_CLEARNET` to allow direct network access without Tor while developing. Never enable it in release builds.
