# sejilochat Privacy Policy

## Current code reference

This policy documents the existing native application under `Sejilo/`. The
Windows/Android Flutter foundation in `sejilo_chat/` has not implemented BLE,
encryption, or persistent storage and must not be represented as a released
private messenger.
...
sejilochat is designed for private, account-free communication. This policy describes what the app keeps on your device, what it sends when you use mesh or optional internet features, and how long local data can remain.
...
## What sejilochat Stores on Your Device
...
1. **Identity and cryptographic keys**
   ...
   - Keys remain until they are rotated, removed by the relevant feature, or erased with panic wipe. Because operating-system keychains can outlive an uninstall, sejilochat records a non-secret install marker and deletes surviving app keys before use after a later reinstall.
...
7. **Optional location-channel state**
   ...
   - sejilochat does not persist exact latitude or longitude and does not include exact coordinates in mesh or Nostr messages.

## Temporary Session Data

While running, sejilochat maintains active connections, routing state, deduplication state, and bounded in-memory conversation timelines. Closing the app clears the in-memory timelines and active connections, but it does not erase the persistent stores listed above.
...
- Private fallback messages use SejiloChat's app-specific encrypted envelopes. This format is not NIP-17, NIP-44, or NIP-59 compatible. Relays can observe the recipient public-key tag, event timing and size, and network metadata, but not the message plaintext or stable sender identity.
...
- Exact coordinates are not included in sejilochat mesh or Nostr payloads and are not persisted by sejilochat.
- A selected geohash can still reveal an approximate area to peers and relays.
- When sejilochat asks the operating system for a friendly place name, Apple's `CLGeocoder` service may process the location under Apple's privacy terms.
...
- These permissions can be revoked in system settings. sejilochat does not record microphone or camera input while the related capture UI is inactive.
...
- Nostr events use secp256k1 Schnorr signatures. SejiloChat private envelopes use secp256k1 key agreement, HKDF-SHA256, and XChaCha20-Poly1305. The envelope format is proprietary, only interoperates with SejiloChat clients, and does not provide forward secrecy against later compromise of the recipient's static Nostr private key.
...
- Include exact GPS coordinates in sejilochat mesh or Nostr message payloads.
...
sejilochat is an open source project. For privacy questions:

- View the source: [https://github.com/permissionlesstech/sejilochat](https://github.com/permissionlesstech/sejilochat)
