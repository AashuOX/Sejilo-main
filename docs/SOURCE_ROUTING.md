# Source-Based Routing for SejiloChat Packets (v2)

## Current code reference

The native source-routing implementation is in `Sejilo/Services/BLE/`, including
`BLERouteForwardingPolicy.swift` and `BLESourceRouteOriginationPolicy.swift`.
The Flutter transport interface does not implement routing yet.

This document specifies the Source-Based Routing extension (v2) for the SejiloChat protocol. This upgrade enables efficient unicast routing across the mesh by allowing senders to specify an explicit path of intermediate relays.
