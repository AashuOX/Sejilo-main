# sejilochat Privacy Assessment

## Current code reference

This assessment covers the legacy Swift application under `Sejilo/`. The new
Windows/Android foundation in `sejilo_chat/` is not ready for private
communication because encryption and persistent secure storage are unimplemented.
...
The app does not advertise the device's assigned name. iOS manages BLE address randomization; sejilochat does not attempt to create a stable MAC address.

That randomization does not deliver the unlinkability it might suggest, because the application layer publishes stable identifiers above it. The 8-byte peer ID in every packet header is the first 8 bytes of the Noise static key fingerprint, so it does not rotate; announces carry the static keys themselves; and the fixed service UUID makes any sejilochat device detectable as such by a passive scanner. A receiver in radio range can therefore recognise a specific device across sessions and locations, and detect that the app is in use at all. RSSI, timing, traffic volume, and radio fingerprints remain observable as well.
...
- SejiloChat's proprietary private-envelope fallback protects plaintext with secp256k1 key agreement, HKDF-SHA256, and XChaCha20-Poly1305. It is not NIP-17, NIP-44, or NIP-59 compatible and does not provide forward secrecy against later compromise of the recipient's static Nostr private key. Relays still see the recipient public-key tag, event timing and size, and network metadata.
...
- Exact coordinates are not persisted by sejilochat or placed into mesh/Nostr payloads.
...
`sejilochat/PrivacyInfo.xcprivacy` declares:
...
`sejilochatShareExtension/PrivacyInfo.xcprivacy` declares app-group UserDefaults reason `1C8F.1`. Both manifests declare no tracking domains and no data collection by the app developer. They must remain bundled in their respective executable bundles.
